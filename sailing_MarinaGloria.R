...
saildis<-apply(pathogen,1,simulator)
names(saildis)<-pathogen$org

#function to generate mean(riskd) for each pathogen

pathriskd<-function(sailmc){
  mean(sailmc$riskd)
}

meanrisk<-lapply(saildis,pathriskd)
...
#names(riskdiarrhea)<-"Diarrhea Risk"
#print(riskdiarrhea)

totalr<-replicate(n=10000,expr=1-((1-sample(saildis[[1]]$riskd,size=1))*(1-sample(saildis[[2]]$riskd,size=1))*(1-sample(saildis[[3]]$riskd,size=1))*(1-sample(saildis[[4]]$riskd,size=1))*(1-sample(saildis[[5]]$riskd,size=1))*(1-sample(saildis[[6]]$riskd,size=1))
                 
),simplify = "vector")

#simulate_risktsail<-replicate(n=10000,expr=risktsail,simplify="vector")

#mean_risktsail<-mean(simulate_risktsail)
